...
public class AgendaController {
    ...
    @GetMapping("/")
    public ResponseEntity<List<Agenda>> buscaTodosDrogariasControl(){
    ...
    public ResponseEntity<Agenda> salvaAgendasControl(@RequestBody Agenda agenda){
    	Agenda salvaAgenda = agendaService.salvaAgendas(agenda);
    	return ResponseEntity.status(HttpStatus.CREATED).body(salvaAgenda);
    }
    @PutMapping("/{id}")
    public ResponseEntity<Agenda> alteraAgendasControl(@PathVariable Long id, @RequestBody Agenda agenda){
    	Agenda alteraAgenda = agendaService.alterarAgendas(id, agenda);
    ...
    public ResponseEntity<String> apagaAgendaControl(@PathVariable Long id){
    	boolean apagar = agendaService.apagarAgendas(id);
    	if(apagar) {
    		return ResponseEntity.ok().body("A agenda foi excluida!");
    	}
    	else {
    		return ResponseEntity.notFound().build();
    	}
